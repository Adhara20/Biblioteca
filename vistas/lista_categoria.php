...
// --- CONTROLADOR ---
include('../controladores/filtrar_categorias.php'); // ← Aquí se definen $resultado
...
<body>
  ...
  <div class="px-10 mb-6 flex justify-between items-center">
    <h1 class="titulos">Listas Categorias</h1>
  ...
  <section class="grid-listas">
    <?php if (!empty($resultado)): ?>
      <?php foreach ($resultado as $fila): 
        $nombre = htmlspecialchars($fila['nombreCategoria']);
		// Traducir Estatus
        if ($fila["estatus"] === 'A') {
        $estatus = 'Activo';
        } else{
        ...
        $img = htmlspecialchars($fila['iconoCategoria'] ?? '');
      ?>
      ...
      <!-- Detalles -->
       <!-- cambiar la ruta del archivo en  href y el pk-->
        ...
        <?php if (($fila['estatus'] ?? '') === 'A'): ?>
          <!-- Desactivar -->
           <!-- cambiar la ruta del archivo en  href y el pk-->
          <a href="../controladores/desactivar_categoria.php?pkCategoria=<?= $fila['pkCategoria'] ?>"
        ...
        <?php else: ?>
          <!-- cambiar la ruta del archivo en  href y el pk-->
            <a href="../controladores/activar_categoria.php?pkCategoria=<?= $fila['pkCategoria'] ?>"
